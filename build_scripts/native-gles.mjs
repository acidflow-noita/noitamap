...
// Declare once: repeated preflights must not lose device enumeration because
...
export function createNativeGLES({ requireHardware = false } = {}) {
  ...
  const softwareRenderer = (name) => /llvmpipe|softpipe|swiftshader|lavapipe|software|basic render/i.test(name || "");
  const candidates = [];
  // NVIDIA's headless graphics path uses EGL_EXT_platform_device. Mesa's
  // surfaceless platform alone is not a portable GPU-runner implementation.
  try {
    const address = egl.func("void *eglGetProcAddress(const char *name)")("eglQueryDevicesEXT");
    if (address) {
      const devices = new Array(16).fill(null), count = [0];
      if (koffi.call(address, QUERY_DEVICES, devices.length, devices, count))
        for (const device of devices.slice(0, count[0])) candidates.push([0x313f, device, "device"]);
    }
  } catch { /* EXT entrypoint optional; try surfaceless below. */ }
  candidates.push([0x31dd, null, "surfaceless"]);
  let display = null, surface = null, context = null, platformName = "";
  const failures = [];
  for (const [kind, device, label] of candidates) {
    const d = platform(kind, device, null);
    if (!d || !initialize(d, [0], [0])) { failures.push(`${label}: EGL initialization failed`); continue; }
    const config = [null], count = [0];
    if (!choose(d, [0x3033, 1, 0x3040, 0x40, 0x3024, 8, 0x3023, 8, 0x3022, 8, 0x3021, 8, 0x3038], config, 1, count) || !count[0]) {
      failures.push(`${label}: no GLES3 pbuffer config`); terminate(d); continue;
    }
    bind(0x30a0);
    const s = makeSurface(d, config[0], [0x3057, 2048, 0x3056, 2048, 0x3038]);
    const c = s ? makeContext(d, config[0], null, [0x3098, 3, 0x3038]) : null;
    if (!c || !makeCurrent(d, s, s, c)) {
      if (c) destroyContext(d, c); if (s) destroySurface(d, s); terminate(d);
  ...
  const nativePut = createCanvas(1, 1).getContext("2d").constructor.prototype
    .putImageData;
  const renderer = getString(0x1f01);
  const vendor = getString(0x1f00);
  const version = getString(0x1f02);
  ...
  return {
    renderer,
    info: { renderer, vendor, version, hardware: !software, platform: platformName },
    get draws() {
      return draws;
    },
