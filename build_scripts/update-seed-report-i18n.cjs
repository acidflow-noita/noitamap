...
const TRANSLATIONS = {
  ...
  compareVs: {
    en: "compared to {{target}} (seed {{seed}})",
    br: "em comparação com {{target}} (semente {{seed}})",
    cs: "ve srovnání s {{target}} (semínko {{seed}})",
    de: "im Vergleich zu {{target}} (Seed {{seed}})",
    es: "comparado con {{target}} (semilla {{seed}})",
    fi: "verrattuna {{target}} (siemen {{seed}})",
    fr: "comparé à {{target}} (graine {{seed}})",
    id: "dibandingkan dengan {{target}} (benih {{seed}})",
    it: "rispetto a {{target}} (seme {{seed}})",
    ja: "{{target}}との比較 (シード {{seed}})",
    nl: "vergeleken met {{target}} (seed {{seed}})",
    pl: "w porównaniu do {{target}} (ziarno {{seed}})",
    ru: "в сравнении с {{target}} (сид {{seed}})",
    sv: "jämfört med {{target}} (frö {{seed}})",
    uk: "у порівнянні з {{target}} (сід {{seed}})",
    zh: "与 {{target}} 对比 (种子 {{seed}})",
  },
  compareUnavailable: {
  ...
  compareNotCached: {
    en: "compared to {{target}} (seed {{seed}}) - visit that seed once to load comparison data",
    br: "em comparação com {{target}} (semente {{seed}}) - visite essa semente uma vez para carregar os dados de comparação",
    cs: "ve srovnání s {{target}} (semínko {{seed}}) - navštivte to semínko jednou pro načtení srovnání",
    de: "im Vergleich zu {{target}} (Seed {{seed}}) - besuche diesen Seed einmal, um Vergleichsdaten zu laden",
    es: "comparado con {{target}} (semilla {{seed}}) - visita esa semilla una vez para cargar los datos de comparación",
    fi: "verrattuna {{target}} (siemen {{seed}}) - käy kyseisessä siemenessä kerran ladataksesi vertailutiedot",
    fr: "comparé à {{target}} (graine {{seed}}) - visitez cette graine une fois pour charger les données de comparaison",
    id: "dibandingkan dengan {{target}} (benih {{seed}}) - kunjungi benih itu sekali untuk memuat data perbandingan",
    it: "rispetto a {{target}} (seme {{seed}}) - visita quel seme una volta per caricare i dati di confronto",
    ja: "{{target}}との比較 (シード {{seed}}) - そのシードを一度訪れて比較データを読み込んでください",
    nl: "vergeleken met {{target}} (seed {{seed}}) - bezoek die seed één keer om vergelijkingsgegevens te laden",
    pl: "w porównaniu do {{target}} (ziarno {{seed}}) - odwiedź to ziarno raz, aby załadować dane porównawcze",
    ru: "в сравнении с {{target}} (сид {{seed}}) - откройте этот сид один раз, чтобы загрузить данные сравнения",
    sv: "jämfört med {{target}} (frö {{seed}}) - besök det fröet en gång för att läsa in jämförelsedata",
    uk: "у порівнянні з {{target}} (сід {{seed}}) - відкрийте цей сід один раз, щоб завантажити дані порівняння",
    zh: "与 {{target}} 对比 (种子 {{seed}}) - 访问一次该种子以加载对比数据",
  },
  ...
  // Locked banner.
  lockedTitle: {
    en: "Full seed report is a Pro feature",
    br: "O relatório completo da semente é um recurso Pro",
    cs: "Úplný přehled seedu je funkce Pro",
    de: "Der vollständige Seed-Bericht ist eine Pro-Funktion",
    es: "El informe completo de semilla es una función Pro",
    fi: "Täysi siemenraportti on Pro-ominaisuus",
    fr: "Le rapport complet de graine est une fonctionnalité Pro",
    id: "Laporan seed lengkap adalah fitur Pro",
    it: "Il rapporto completo del seme è una funzionalità Pro",
    ja: "完全なシードレポートはProの機能です",
    nl: "Het volledige seedrapport is een Pro-functie",
    pl: "Pełny raport ziarna to funkcja Pro",
    ru: "Полный отчёт по сиду — функция Pro",
    sv: "Den fullständiga frörapporten är en Pro-funktion",
    uk: "Повний звіт про сід — це функція Pro",
    zh: "完整种子报告是 Pro 功能",
  },
  lockedBody: {
